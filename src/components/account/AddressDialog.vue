    ...
    <form @submit="onSubmit" class="mt-1 text-base space-y-6">
      ...
      <div class="w-fit">
        <div
          ...
          :class="{
            'opacity-50': props.forcePrimaryAddress || currentAddress?.is_primary,
            'cursor-not-allowed': props.forcePrimaryAddress || currentAddress?.is_primary,
            'cursor-pointer': !props.forcePrimaryAddress && !currentAddress?.is_primary,
          }"
          v-tooltip.top="
            props.forcePrimaryAddress || currentAddress?.is_primary
              ? {
                  value:
        ...
        >
          <Checkbox
            ...
            inputId="primaryAddress"
            binary
            :disabled="props.forcePrimaryAddress || currentAddress?.is_primary"
            class="w-6"
          />
          <label
            for="primaryAddress"
      ...
      <div v-if="authStore.user?.role === 'seller' || forcePickupAddress === true" class="w-fit">
        <div
          class="flex items-center min-w-fit"
          :class="{
            'opacity-50': props.forcePickupAddress || currentAddress?.is_pickup_address,
            'cursor-not-allowed': props.forcePickupAddress || currentAddress?.is_pickup_address,
            'cursor-pointer': !props.forcePickupAddress && !currentAddress?.is_pickup_address,
          }"
          v-tooltip.top="
            props.forcePickupAddress || currentAddress?.is_pickup_address
              ? {
                  value:
                    'Bạn không thể xoá nhãn Địa chỉ lấy hàng. Hãy đặt địa chỉ khác làm Địa chỉ lấy hàng của bạn nhé.',
                  pt: {
                    root: {
                      style: { 'max-width': '170px', 'font-size': '0.875rem' },
                    },
        ...
        >
          <Checkbox
            v-model="isPickupAddress"
            inputId="pickupAddress"
            binary
            :disabled="props.forcePickupAddress || currentAddress?.is_pickup_address"
            class="w-6"
          />
          <label
            for="pickupAddress"
...
const validationSchema = yup.object({
  ...
  province: yup.number().required('Vui lòng chọn Tỉnh/Thành phố'),
  district: yup.number().required('Vui lòng chọn Quận/Huyện'),
...
const dialogTitle = computed(() =>
  dialogMode.value === 'create' ? 'Địa chỉ mới' : 'Cập nhật địa chỉ',
)
  ...
  if (mode === 'update' && address) {
    ...
    try {
      ...
      const provinceObj = provinces.value.find(
        (p) => p.ProvinceName.trim() === address.province_name?.trim(),
      )
...
const onSubmit = handleSubmit(
  async (values) => {
    try {
      ...
      console.error(
        `Error ${dialogMode.value === 'create' ? 'creating' : 'updating'} address:`,
        error,
      )
    }
  },
