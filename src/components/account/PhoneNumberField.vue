...
<template>
  ...
  <div v-if="currentPhoneNumber" class="flex items-center gap-2 text-sm">
    ...
    <button
      ...
      @click="showUpdatePhoneNumberDialog = true"
    >
      Thay Đổi
    </button>
  </div>
  ...
  <Dialog
    v-model:visible="showUpdatePhoneNumberDialog"
    modal
  ...
  >
    <form @submit="proceedToStep2">
      ...
      <div class="mb-4">
        ...
        <div class="h-8 mt-1">
          <!-- h-5 = 20px height, enough for one line of text -->
  ...
  <Dialog
    v-model:visible="showOTPAskingDialog"
    modal
    header="Cập nhật số điện thoại"
    :style="{ width: '21rem' }"
    class="rounded-sm"
  ...
  <!-- Step 3: Submit OTP -->
  <Dialog
    v-model:visible="showOTPInputDialog"
    modal
    header="Cập nhật số điện thoại"
    :style="{ width: '24rem' }"
    ...
    <form @submit.prevent="handleOTPSubmit" class="flex flex-col h-full justify-between">
      ...
      <div class="text-center mb-6">
        ...
        >
          {{
            timeLeft > 0
              ? `Vui lòng chờ ${formatDuration({ seconds: timeLeft }, { locale: vi })} để gửi lại`
              : 'Gửi lại'
          }}
        </button>
      </div>
...
const newPhoneNumberSchema = yup.object({
  newPhoneNumber: yup
    ...
    .test(
      'is-not-same-as-old-phone',
      'Số điện thoại mới không được trùng với số điện thoại cũ',
      (newPhoneNumber) => {
